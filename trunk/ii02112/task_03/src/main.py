...
def draw_vertex():
    mainLabel.configure(text="Вы выбрали рисование вершин, нажмите на свободное место для рисования вершины")
...
def draw_edge():
    mainLabel.configure(text="Вы выбрали рисование ребер, нажмите на вершину для рисования ребра")
    canvas.unbind("<Button-1>")
...
def delete_vertex():
    mainLabel.configure(text="Вы выбрали удаление вершины или ребра, нажмите на вершину или ребро для удаления")
    canvas.unbind("<Button-1>")
...
def rename_vertex():
    mainLabel.configure(text="Вы выбрали переименование вершины, нажмите на вершину для переименования")
    canvas.unbind("<Button-1>")
...
def change_color():
    mainLabel.configure(text="Вы выбрали изменение цвета вершины, нажмите на вершину для изменения её цвета")
    canvas.unbind("<Button-1>")
...
def change_text_color():
    mainLabel.configure(text="Вы выбрали изменение цвета текста, нажмите на вершину для изменения цвета её текста")
    canvas.unbind("<Button-1>")
...
def change_edge_color():
    mainLabel.configure(text="Вы выбрали изменение цвета ребер, нажмите на ребро для изменения его цвета")
    canvas.unbind("<Button-1>")
    ...
    if click_num == 0:
        for x in ovals:
            if canvas.find_overlapping(event.x, event.y, event.x, event.y)[0] == x:  # проверка клика на вершину
                ...
                click_num = 1
                mainLabel.configure(text="Выберите вторую вершину для рисования ребра")
    elif click_num == 1:
        for x in ovals:
            if canvas.find_overlapping(event.x, event.y, event.x, event.y)[0] == x:
...
def adjacency_matrix():
    ...
    for i3 in range(len(matrix)):
        adj_matrix_label = Label(adj_matrix, text=str(matrix[i3]))
...
def incidence_matrix():
    ...
    for index2 in range(len(matrix)):
        inc_matrix_label = Label(inc_matrix, text=str(matrix[index2]))
...
root = Tk()
root.title("Graph")
mainLabel = Label(root, text="Выберите действие")
...
Tk.geometry(root, "800x600")
canvas = Canvas(root, width=1920, height=1080)  # основной canvas
canvas.pack()
menubar = Menu(root)  # меню

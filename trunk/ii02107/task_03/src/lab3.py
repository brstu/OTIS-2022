...
data_ID_nonorient_line_X_Y = dict() # словарь для хранения ID неориентированных рёбер и для имён и координат вершин с которыми они соединены
    ...
    for i in data_vertex_id_X_Y:
        if mouse_x > data_vertex_id_X_Y[i][0] - 15 and mouse_x < data_vertex_id_X_Y[i][0] + 15 and \
            ...
            if non_oriented_line == 0:
                ...
                data_ID_nonorient_line_X_Y[ID_none_oriented_line] = [[x1,y1,name1,color],[x2,y2,name2,color]]
                print("data_ID_nonorient_line_X_Y\t",data_ID_nonorient_line_X_Y)
    ...
    for i in data_vertex_id_X_Y:
        if mouse_x > data_vertex_id_X_Y[i][0] - 15 and mouse_x < data_vertex_id_X_Y[i][0] + 15 and\
            ...
            if non_oriented_line == 0:
                ...
                count_unoriented_line-=1
                for key in data_ID_nonorient_line_X_Y:
                    if data_ID_nonorient_line_X_Y[key][0][0] == x1 and data_ID_nonorient_line_X_Y[key][0][1] == y1 and\
                         data_ID_nonorient_line_X_Y[key][1][0] == x2 and data_ID_nonorient_line_X_Y[key][1][1] == y2:
                        print("data_ID_nonorient_line_X_Y in delete\t",data_ID_nonorient_line_X_Y)
                        canvas.create_line(*line_intersect_circle(x1, y1, x2, y2), fill="white", width=3)
                        del data_ID_nonorient_line_X_Y[key]
                        break
                    elif data_ID_nonorient_line_X_Y[key][0][0] == x2 and data_ID_nonorient_line_X_Y[key][0][1] == y2 and\
                         data_ID_nonorient_line_X_Y[key][1][0] == x1 and data_ID_nonorient_line_X_Y[key][1][1] == y1:
                        print("data_ID_nonorient_line_X_Y in delete\t",data_ID_nonorient_line_X_Y)
                        canvas.create_line(*line_intersect_circle(x1, y1, x2, y2), fill="white", width=3)
                        del data_ID_nonorient_line_X_Y[key]
                        break
    ...
    for i in data_vertex_id_X_Y:
        if mouse_x > data_vertex_id_X_Y[i][0] - 15 and mouse_x < data_vertex_id_X_Y[i][0] + 15 and\
            ...
            if oriented_line == 0:
                ...
                canvas.create_line(*line_intersect_circle(x1, y1, x2, y2), fill=color1, width=2, arrow=LAST)
                oriented_line = 0
                break
    ...
    for i in data_vertex_id_X_Y:
        if mouse_x > data_vertex_id_X_Y[i][0] - 15 and mouse_x < data_vertex_id_X_Y[i][0] + 15 and\
            ...
            if oriented_line == 0:
                ...
                canvas.create_line(*line_intersect_circle(x1, y1, x2, y2), fill="white", width=3, arrow=LAST)
                oriented_line = 0
                break
...
def input_weight(): # рисование веса
    ...
    label1 = Label(new_application1, text="Введите вес ребра")
    label1.grid(row=0, column=0, sticky="ew")
    entry1 = Entry(new_application1)
    entry1.grid(row=1, column=0,sticky="ew")
    button1 = Button(new_application1, text="OK", command=lambda: input_weight_on_click(entry1.get(),new_application1))
...
def delete_vertex_on_click(event):
    ...
    for i in data_vertex_id_X_Y:
        if mouse_x > data_vertex_id_X_Y[i][0] - 15 and mouse_x < data_vertex_id_X_Y[i][0] + 15 and\
            ...
            canvas.create_oval(data_vertex_id_X_Y[i][0] - 15, data_vertex_id_X_Y[i][1] - 15, data_vertex_id_X_Y[i][0] + 15, data_vertex_id_X_Y[i][1] + 15,fill="white", outline="white", width=2)
            name_delete_vertex = data_vertex_id_X_Y[i][2]
            
            print("словарь вершин:\t",data_vertex_id_X_Y[i][2])
            if ID_none_oriented_line !=0:
                for name in data_ID_nonorient_line_X_Y:
                    if data_ID_nonorient_line_X_Y[name][0][2] == name_delete_vertex or data_ID_nonorient_line_X_Y[name][1][2] == name_delete_vertex:
                        canvas.create_line(*line_intersect_circle(data_ID_nonorient_line_X_Y[name][0][0], data_ID_nonorient_line_X_Y[name][0][1],\
                             data_ID_nonorient_line_X_Y[name][1][0], data_ID_nonorient_line_X_Y[name][1][1]), fill="white", width=2)
                        canvas.delete(data_ID_nonorient_line_X_Y[name])
                        
                        all_delete_name1.append(name)
                for j in all_delete_name1:
                    del data_ID_nonorient_line_X_Y[j]
                print("data_ID_nonorient_line_X_Y:\t",data_ID_nonorient_line_X_Y)
                ...
                for name in data_ID_orient_line_X_Y:
                    if data_ID_orient_line_X_Y[name][0][2] == name_delete_vertex or data_ID_orient_line_X_Y[name][1][2] == name_delete_vertex:
                        canvas.create_line(*line_intersect_circle(data_ID_orient_line_X_Y[name][0][0], data_ID_orient_line_X_Y[name][0][1],\
                            data_ID_orient_line_X_Y[name][1][0], data_ID_orient_line_X_Y[name][1][1]), fill="white", width=2)
...
def vanish_vertex_on_click(event):
    ...
    for i in data_vertex_id_X_Y: # delete vertex and line with this vertex
        if mouse_x > data_vertex_id_X_Y[i][0] - 15 and mouse_x < data_vertex_id_X_Y[i][0] + 15 and\
            ...
            print("словарь вершин:\t",data_vertex_id_X_Y[i][2])
            if ID_none_oriented_line !=0:
                for id in data_ID_nonorient_line_X_Y:
                    if data_ID_nonorient_line_X_Y[id][0][2] == name_vanish_vertex or data_ID_nonorient_line_X_Y[id][1][2] == name_vanish_vertex:
                        canvas.create_line(*line_intersect_circle(data_ID_nonorient_line_X_Y[id][0][0], data_ID_nonorient_line_X_Y[id][0][1],\
                             data_ID_nonorient_line_X_Y[id][1][0], data_ID_nonorient_line_X_Y[id][1][1]), fill="white", width=2)
                        all_vanish_nonorline_id.append(id)
                print("data_ID_nonorient_line_X_Y:\t",data_ID_nonorient_line_X_Y)

            if ID_oriented_line !=0:
                for id in data_ID_orient_line_X_Y:
                    if data_ID_orient_line_X_Y[id][0][2] == name_vanish_vertex or data_ID_orient_line_X_Y[id][1][2] == name_vanish_vertex:
                        canvas.create_line(*line_intersect_circle(data_ID_orient_line_X_Y[id][0][0], data_ID_orient_line_X_Y[id][0][1],\
                             data_ID_orient_line_X_Y[id][1][0], data_ID_orient_line_X_Y[id][1][1]), fill="white", width=2, arrow=LAST)
                        all_vanish_orline_id.append(id)
                print("data_ID_orient_line_X_Y:\t",data_ID_orient_line_X_Y)
...
def appearance_vertex_on_move(event):
    ...
    if mouse_y>0:
        ...
        print("data_vertex_id_X_Y:\t",data_vertex_id_X_Y)
        if ID_none_oriented_line !=0:
            for id in all_vanish_nonorline_id:
                if name_vanish_vertex == data_ID_nonorient_line_X_Y[id][0][2]:
                    data_ID_nonorient_line_X_Y[id][0] = [mouse_x, mouse_y, name_vanish_vertex,data_ID_nonorient_line_X_Y[id][0][3]]
                    canvas.create_line(*line_intersect_circle(data_ID_nonorient_line_X_Y[id][0][0], data_ID_nonorient_line_X_Y[id][0][1],\
                         data_ID_nonorient_line_X_Y[id][1][0], data_ID_nonorient_line_X_Y[id][1][1]), fill=data_ID_nonorient_line_X_Y[id][0][3], width=2)
                elif name_vanish_vertex == data_ID_nonorient_line_X_Y[id][1][2]:
                    data_ID_nonorient_line_X_Y[id][1] = [mouse_x, mouse_y, name_vanish_vertex,data_ID_nonorient_line_X_Y[id][1][3]]
                    canvas.create_line(*line_intersect_circle(data_ID_nonorient_line_X_Y[id][0][0], data_ID_nonorient_line_X_Y[id][0][1],\
                         data_ID_nonorient_line_X_Y[id][1][0], data_ID_nonorient_line_X_Y[id][1][1]), fill=data_ID_nonorient_line_X_Y[id][1][3], width=2)
            print("data_ID_nonorient_line_X_Y:\t",data_ID_nonorient_line_X_Y)
        if ID_oriented_line !=0:
            for id in all_vanish_orline_id:
                if name_vanish_vertex == data_ID_orient_line_X_Y[id][0][2]:
                    data_ID_orient_line_X_Y[id][0] = [mouse_x, mouse_y, name_vanish_vertex,data_ID_orient_line_X_Y[id][0][3]]
                    canvas.create_line(*line_intersect_circle(data_ID_orient_line_X_Y[id][0][0], data_ID_orient_line_X_Y[id][0][1],\
                         data_ID_orient_line_X_Y[id][1][0], data_ID_orient_line_X_Y[id][1][1]), fill=data_ID_orient_line_X_Y[id][0][3], width=2, arrow=LAST)
                elif name_vanish_vertex == data_ID_orient_line_X_Y[id][1][2]:
                    data_ID_orient_line_X_Y[id][1] = [mouse_x, mouse_y, name_vanish_vertex,data_ID_orient_line_X_Y[id][1][3]]
                    canvas.create_line(*line_intersect_circle(data_ID_orient_line_X_Y[id][0][0], data_ID_orient_line_X_Y[id][0][1],\
                         data_ID_orient_line_X_Y[id][1][0], data_ID_orient_line_X_Y[id][1][1]), fill=data_ID_orient_line_X_Y[id][1][3], width=2, arrow=LAST)
            print("data_ID_orient_line_X_Y:\t",data_ID_orient_line_X_Y)
...
def rename_vertex_name(name, root, i):
    ...
    if name == "":
        ...
        if ID_none_oriented_line != 0: # при изменнии имени вершины, делаем проверку на нахождение переименованной вершины в словаре неориентированных ребер
            for i in data_ID_nonorient_line_X_Y: # и если находим, то меняем имя вершины в списке ребер на новое имя
                if data_ID_nonorient_line_X_Y[i][0][2] == rename_name:
                    data_ID_nonorient_line_X_Y[i][0][2] = name

                if data_ID_nonorient_line_X_Y[i][1][2] == rename_name:
                    data_ID_nonorient_line_X_Y[i][1][2] = name  
            print("data_ID_orient_line_X_Y\t",data_ID_nonorient_line_X_Y)
...
def algorithm(): # функция для создания окна с алгоритмами
    ...
    if UNORIN_ORIENT ==0:
        button_diametr_graph = Button(new_application, text="Диаметр графа", command=lambda: diameterGraph(nodes,temp_edges),width=12, height=2)
        button_radius_graph = Button(new_application, text="Радиус графа", command=lambda: radiusGraph(nodes,temp_edges),width=12, height=2)
        button_center_graph = Button(new_application, text="Центр графа", command=lambda: centerGraph(nodes,temp_edges),width=12, height=2)
    else:
        button_diametr_graph = Button(new_application, text="Диаметр графа", command=lambda: diameterGraph(nodes,temp_edges),width=12, height=2, state=DISABLED)
        button_radius_graph = Button(new_application, text="Радиус графа", command=lambda: radiusGraph(nodes,temp_edges),width=12, height=2, state=DISABLED)
        button_center_graph = Button(new_application, text="Центр графа", command=lambda: centerGraph(nodes,temp_edges),width=12, height=2, state=DISABLED)
...
def main():
    ...
    button_draw_line_between_vertex = Button(application, text="draw unoriented_line\n between vertex",\
         command=lambda: draw_unoriented_line_between_vertex(),width=15, height=2)
    button_draw_oriented_line_between_vertex = Button(application, text="draw oriented\n  between vertex",\
         command=lambda: draw_oriented_line_between_vertex(),width=15, height=2)
    ...
    button_weight_line = Button(application, text="input weight\n line", command=lambda: input_weight(),width=15, height=2)
    button_draw_weight_line = Button(application, text="draw weight\n line", command=lambda: draw_weight_line(),width=15, height=2)
    button_delete_weight_line = Button(application, text="delete weight\n line", command=lambda: delete_weight_line(),width=15, height=2)
    button_NEW_window = Button(application, text="NEW window", command=lambda: NEW_window(),width=15, height=2)
    ...
    button_NEW_window.grid(row=2, column=2)

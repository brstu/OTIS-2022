using Plots
using LinearAlgebra
function lineral_model(time,desired_temp=30)
    warm=1
    array_of_e=[0.001,0.19,0.00002]
    array_of_q=[0.000004,0.0000011,0.000014]
    weigth=[1,0,1,1.0]
    y=1
    prev_warm=1
    result=[]
    for i in 1:time
        ...
        array_of_e[3]=desired_temp-y
        y=weigth[1]*y+weigth[2]*warm
        array_of_e[2]=desired_temp-y
        y=weigth[1]*y+weigth[2]*warm
        array_of_e[1]=desired_temp-y
        warm=prev_warm+dot(array_of_q,array_of_e)
        prev_warm=warm
        push!(result,y)
    end
    return result
end
function nonlineral_model(param,time,start_warm=1,desired_temp=30)
    result=[]
    array_of_e=[0.001,0.19,0.00002]
